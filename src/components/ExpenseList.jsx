...
const ExpenseList = ({ expenses, sortedExpenses, handleDeleteExpense, handleEditExpense }) => {
  ...
  return (
    <section className="bg-transparent md:bg-white md:p-8 md:rounded-2xl md:shadow-sm md:border md:border-gray-100 overflow-hidden">
      
      {/* 📱 MOBILE VIEW: Stacked Cards (Visible under 768px, 0 horizontal scroll) */}
      <div className="md:hidden flex flex-col gap-4">
        {sortedExpenses.length > 0 ? (
          sortedExpenses.map(expense => (
            <div key={expense.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 relative group overflow-hidden flex flex-col gap-3 transition-shadow hover:shadow-md">
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-1">
                  <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold bg-primary-50 text-primary-700 border border-primary-100/50 w-max shadow-sm">
                    {expense.category}
                  </span>
                  <div className="text-xs font-semibold text-gray-500 mt-1">{expense.date}</div>
                </div>
                <div className="text-xl font-black text-gray-900 absolute top-5 right-5">
                  ₹{expense.amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </div>
              </div>

              {expense.note && (
                <div className="text-sm text-gray-600 border-l-2 border-gray-200 pl-3 py-1 font-medium italic truncate">
                  "{expense.note}"
                </div>
              )}

              <div className="pt-3 border-t border-gray-50 flex items-center justify-between mt-1">
                <div className="flex items-center gap-1 opacity-90 w-full overflow-hidden">
                  <span className="text-[10px] text-gray-400 font-mono tracking-wider truncate" title={expense.id}>
                    ID: {expense.id.toString().substring(0, 8)}...
                  </span>
                  <button 
                    onClick={() => handleCopyId(expense.id)}
                    className={`p-1.5 rounded flex items-center justify-center transition-colors ${copiedId === expense.id ? 'text-green-500 bg-green-50' : 'text-gray-400 hover:text-primary-500 hover:bg-primary-50'}`}
                  >
                    {copiedId === expense.id ? (
                ...
                <div className="flex items-center gap-1 justify-end shrink-0 pl-4 border-l border-gray-100">
                  <button onClick={() => handleEditExpense(expense.id)} className="text-gray-300 hover:text-amber-500 p-2 rounded-xl hover:bg-amber-50 focus:ring-2 focus:ring-amber-200" title="Edit">
                  ...
                  <button onClick={() => handleDeleteExpense(expense.id)} className="text-gray-300 hover:text-red-500 p-2 rounded-xl hover:bg-red-50 focus:ring-2 focus:ring-red-200" title="Delete">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100">{renderEmptyState()}</div>
        )}
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[650px]">
          <thead>
            <tr className="border-b-2 border-gray-100 bg-gray-50/50">
              ...
              <th className="py-4 px-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Note</th>
              <th className="py-4 px-5 text-xs font-bold text-gray-400 uppercase tracking-widest text-right">Amount</th>
              <th className="py-4 px-5 text-xs font-bold text-gray-400 uppercase tracking-widest text-center rounded-tr-xl w-24">Action</th>
            </tr>
          </thead>
          <tbody>
            {sortedExpenses.length > 0 ? (
                ...
                <tr key={expense.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/80 transition-all group">
                  <td className="py-4 px-5 whitespace-nowrap">
                    ...
                    <div className="flex items-center gap-1.5 opacity-80 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => handleCopyId(expense.id)} className={`p-1 rounded flex items-center transition-colors ${copiedId === expense.id ? 'text-green-500 bg-green-50' : 'text-gray-400 hover:text-primary-500 hover:bg-primary-50'}`} title="Copy ID">
                  ...
                  <td className="py-4 px-5 whitespace-nowrap">
                    <span className="inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold bg-primary-50 text-primary-700 border border-primary-100/50 shadow-sm">{expense.category}</span>
                  </td>
                  <td className="py-4 px-5 text-sm text-gray-500 max-w-[180px] truncate" title={expense.note}>
                  ...
                  <td className="py-4 px-5 text-center whitespace-nowrap">
                    <button onClick={() => handleEditExpense(expense.id)} className="text-gray-300 hover:text-amber-500 p-2 rounded-xl hover:bg-amber-50 transition-all opacity-0 group-hover:opacity-100 mr-1"><svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>
                    <button onClick={() => handleDeleteExpense(expense.id)} className="text-gray-300 hover:text-red-500 p-2 rounded-xl hover:bg-red-50 transition-all opacity-0 group-hover:opacity-100"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M3 6h18"></path><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path></svg></button>
                  </td>
                </tr>
              ))
            ) : (
